...
unsafe impl GlobalAlloc for Collam {
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, _layout: Layout) {
        if let Some(p) = Unique::new(ptr) {
            dealloc(p.cast::<c_void>());
        }
    }

    unsafe fn alloc_zeroed(&self, layout: Layout) -> *mut u8 {
        match alloc_zeroed(layout) {
...
pub unsafe fn alloc(layout: Layout) -> Option<Unique<c_void>> {
    if layout.size() == 0 {
        return None;
    }
    ...
    let mut block = if let Some(block) = pop(layout.size()) {
        block
...
pub unsafe fn dealloc(ptr: Unique<c_void>) {
    let _lock = MUTEX.lock();
...
pub unsafe fn alloc_zeroed(layout: Layout) -> Option<Unique<c_void>> {
...
pub unsafe fn realloc(ptr: Unique<c_void>, new_size: usize) -> Option<Unique<c_void>> {
